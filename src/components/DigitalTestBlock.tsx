...
export function DigitalTestBlock({ data }: { data: TrickBlockData }) {
  const [open, setOpen] = useState(false);
  const navigate = useNavigate();
  const { loadDigitalQuiz } = useQuizStore();
  const { t } = useTranslation();

  const handleStartTest = () => {
    setOpen(false);
    loadDigitalQuiz(data.id);
    navigate(`/template/${data.id}?type=digital`);
  };
