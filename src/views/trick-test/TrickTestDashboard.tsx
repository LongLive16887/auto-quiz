...
import { TrickTestBlock } from '@/components/TrickTestBlock'

const BLOCKS: TrickBlockData[] = Array.from({ length: 8 }).map((_, index) => ({
  id: index,
  correct_answer: 0,
  wrong_answer: 0,
  skipped_answer: 0,
      ...
      <div className='flex flex-col gap-3.5'>
        ...
        <div className='flex justify-center flex-wrap gap-3.5'>
          {statistics.map(block => (
            <TrickTestBlock key={block.id} data={block} />
          ))}
        </div>
      </div>
    </MainLayout>
...
export default TrickTestDashboard
